...
export default class ForceSimulation {
		...
		this.simulation = d3
			...
			.force(
				...
				d3
					.forceX()
					.x((d) => d.anchor?.x ?? 0)
					.strength((d) => (d.anchor?.type == "soft" ? 1 : 0))
			)
			.force(
				"forceY",
				d3
	...
	setDrag() {
		...
		function dragstarted(e, d) {
			const worldPos = {
				x:
					e.x / d.forceSimulation.worldTransform.k -
					d.forceSimulation.worldTransform.x / d.forceSimulation.worldTransform.k,
				y:
					e.y / d.forceSimulation.worldTransform.k -
					d.forceSimulation.worldTransform.y / d.forceSimulation.worldTransform.k,
			};
			...
			d.fx = e.x;
			d.fy = e.y;
			d.forceSimulation.onDragStartEvent(e, d, worldPos);
		}
		...
		function dragended(e, d) {
			...
			d.fx = null;
			d.fy = null;
			d.forceSimulation.onDragEndEvent(e, d, worldPos);
		}
