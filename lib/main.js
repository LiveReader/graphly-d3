...
export default class ForceSimulation {
	...
	constructor(svg) {
		...
		this.worldTransform = { k: 1, x: this.svg.attr("width") / 2, y: this.svg.attr("height") / 2 };
		...
		this.simulation = d3
			...
			.force(
				...
				d3
					...
					.strength((d) => (!!d.satellite ? 2 : 0) || (d.anchor?.type == "soft" ? 2 : 0))
			)
			.force(
				"forceY",
				d3
	...
	spawnNodes(nodes) {
		for (let i = 0; i < nodes.length; i++) {
			...
			if (!node.spawn) {
				node.x = this.svg.attr("width") / 2;
				node.y = this.svg.attr("height") / 2;
				continue;
			}
