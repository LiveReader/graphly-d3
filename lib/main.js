...
export default class ForceSimulation {
	...
	ticked() {
		this.nodeGroup.selectAll("g.node").attr("transform", (d) => {
			...
			const min_y = -this.worldBoundaries.height / 2;
			const max_y = this.worldBoundaries.height / 2;
			const min_x = -this.worldBoundaries.width / 2;
			const max_x = this.worldBoundaries.width / 2;
	...
	async render(graph) {
		...
		const nodes = this.nodeGroup.selectAll("g.node").data(this.graph.nodes, (d) => d.id);
		nodes
			.enter()
			.append(Node)
			.classed("shadow", true)
		...
		nodes.classed("selected", (d) => d.selected).classed("disabled", (d) => d.disabled);
