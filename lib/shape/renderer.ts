...
export function renderTemplate(template: Template, data: Node, config: RenderConfig): Promise<SVGElement> {
	if (!template) return throwError(`[graphly-d3] No "${data.shape?.type ?? ""}" template provided!`);
	if (!data) return Promise.reject(`[graphly-d3] No data provided!`);
	if (!config) return Promise.reject(`[graphly-d3] No config provided!`);
	...
	if (!template.shapePayload) {
		...
		if (!isValid) {
			console.error(
				`[graphly-d3] Invalid payload for template ${
					data.shape?.type ? '"' + data.shape.type + '"' : ""
				}\n • ${validate.errors
	...
	if (selection === null)
		return throwError(
			`[graphly-d3] Template ${data.shape?.type ? '"' + data.shape.type + '"' : ""} failed rendering!`
		);
