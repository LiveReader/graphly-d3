...
export default class ForceSimulation {
	...
	async render(graph, alpha = 0.05) {
		...
		function renderWrapper(parent, data) {
			...
			nodes.exit().transition().duration(this.transitionDuration ?? 300).attr("opacity", 0).remove();
		...
		link.append("text")
			...
			.attr("dy", "0.35em")
			.classed("label", true);
		link.attr("opacity", 0).transition().duration(this.transitionDuration ?? 300).attr("opacity", 1);
		links.exit().transition().duration(this.transitionDuration ?? 300).attr("opacity", 0).remove();
